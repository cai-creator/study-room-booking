...
| 成员 | 角色 | 负责模块 | 技术侧重 |
|------|------|---------|---------|
| 成员A | 项目负责人/后端 | 用户与权限、数据库设计、API规范制定 | 后端 + 协调 |
| 成员B | 后端开发 | 空间管理（校区/楼栋/楼层/自习室/座位CRUD） | 后端 |
| 成员C | 后端开发 | 预约核心（预约流程、规则引擎、并发控制） | 后端 |
| 成员D | 后端开发 | 座位管控（签到/签退/暂离、定时任务、黑名单） | 后端 |
...
| 接口 | 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|------|
| 登录 | POST | `/api/auth/login` | 用户名密码登录 | 公开 |
| CAS登录 | POST | `/api/auth/cas` | 校园统一身份认证登录 | 公开 |
| 登出 | POST | `/api/auth/logout` | 退出登录 | 所有登录用户 |
| 获取当前用户 | GET | `/api/auth/me` | 获取当前登录用户信息 | 所有登录用户 |
...
| 接口 | 方法 | 路径 | 说明 | 权限 |
|------|------|------|------|------|
| 用户列表 | GET | `/api/users` | 分页查询用户列表 | 管理员 |
| 用户详情 | GET | `/api/users/{id}` | 获取用户详情 | 管理员/本人 |
