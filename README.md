...
| 座位预约页 | 空间管理 + 预约核心 | 自习室筛选、座位实时状态查询、创建预约、可用时段 | 成员B + 成员C |
...
| 预约日期范围 | 当天至后天 | 仅可预约今天、明天、后天3天内的时段 |
| 预约截止时间 | 时段开始后50分钟 | 距离时段开始超过50分钟不可预约该时段 |
| 签到时间限制 | 开始/创建后10分钟内 | 提前预约：时段开始前50分钟可签到，开始后10分钟内必须签到；开始后预约：创建后10分钟内必须签到，超时判定为爽约 |
| 多时段预约 | 支持连续多时段 | 一次可预约多个相邻时段，合并为单条预约记录（开始时间=首时段开始，结束时间=末时段结束），签到/签退/暂离等机制与单时段预约一致 |
