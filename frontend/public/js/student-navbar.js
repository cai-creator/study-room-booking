/**
 * 学生端导航栏脚本（单独文件确保被正常执行，避免 innerHTML 插入 script 不执行的问题）
 * 由所有学生端页面引入：<script src="../../public/js/student-navbar.js"></script>
 */
(function () {
  'use strict';
  ...
  window.initStudentNavbarUI = function () {
    var user = null;
    var userKey = (window.AppConfig && AppConfig.storageKeys && AppConfig.storageKeys.user) || 'study_room_user';
    try {
    ...
    if (user) {
      ...
      if (avatarEl) avatarEl.textContent = (user.realName || user.username || 'U').charAt(0);
