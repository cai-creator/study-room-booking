...
│   │   ├── room-selection.html # 自习室筛选 - 多级筛选、卡片列表、分页
│   │   ├── seat-selection.html # 座位选座 - 座位网格、状态图例、时段选择、预约提交
...
学生端共 5 个页面，路径均在 `student/pages/` 下：

| 页面 | 文件 | 核心功能 | 对接模块 |
|------|------|---------|---------|
| 首页 | `home.html` | 欢迎横幅、快捷入口、校区概览卡片、今日预约预览、推荐自习室列表 | 空间管理 + 看板 |
| 自习室筛选 | `room-selection.html` | 校区/楼栋/楼层/房间类型筛选、关键词搜索、仅看有空座、卡片列表、分页 | 空间管理 |
| 座位选座 | `seat-selection.html` | 自习室座位网格可视化（5种状态颜色）、选中座位信息、时段选择、确认预约 | 预约核心 |
